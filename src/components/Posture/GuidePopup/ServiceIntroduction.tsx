export default function ServiceIntroduction() {
  return (
    <>
      <div className="mb-8 flex gap-2">
        <div className="h-2 w-2 rounded-full bg-align_blue-500"></div>
        <div className="h-2 w-2 rounded-full bg-zinc-300"></div>
      </div>
      <div className="mb-6 flex flex-col items-center">
        <div className="mb-3 text-center text-title font-bold">
          자세공작소는 실시간 모니터링으로 <br />
          바른 자세 유지를 돕는 서비스입니다
        </div>
        <span className="text-body3 text-zinc-500">
          더 세밀한 모니터링을 위해, 최초 1회 자세 기준점 설정이 필요합니다.
        </span>
      </div>
      {/* content */}
      <div className="mb-8 flex h-[172px] gap-4">
        <div className="flex w-80 flex-col items-center rounded-[17px] bg-[#EFEFF0] px-10 py-6">
          <div className="mb-4 h-[24px] w-[24px] rounded-full bg-align_blue-300 text-center text-body1 font-semibold text-white">
            1
          </div>
          <span className="mb-2 text-title font-semibold">바른 자세 취하기</span>
          <div className="text-center font-normal leading-6 text-zinc-500">
            가이드에 따라 <br />
            바른 자세를 취해 주세요.
          </div>
        </div>
        <div className="flex w-80 flex-col items-center rounded-[17px] bg-[#EFEFF0] px-10 py-6">
          <div className="mb-4 h-[24px] w-[24px] rounded-full bg-align_blue-300 text-center text-body1 font-semibold text-white">
            2
          </div>
          <span className="mb-2 text-title font-semibold">스냅샷 촬영하기</span>
          <div className="text-center font-normal leading-6 text-zinc-500">
            기준점 설정을 위해 <br />
            촬영을 진행해 주세요.
          </div>
        </div>
      </div>
    </>
  )
}
